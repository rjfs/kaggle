...
class ToxicRandomForest:
    ...
    def load_data(self, data_path):
        # Load features data
        self.train_features = get_features(data_path + 'train-features.csv')
        self.test_features = get_features(data_path + 'test-features.csv')
    ...
    def predictions_df(self, features):
        ...
        df = pd.DataFrame(
            preds, index=features.index, columns=self.output_classes
        )
        df.index.name = 'id'
...
def get_features(fpath):
    ...
    lower_upper = features['n_lower'] + features['n_upper']
    features['caps_pct'] = features['n_upper'] / lower_upper
    ...
    to_drop = [
        'has_ip', 'n_smilies', 'n_symbols', 'n_stars', 'n_interrogation',
        'n_seps'
    ]
